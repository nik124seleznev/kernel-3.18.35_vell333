...
static int ap3426_dsm_report_err(int errno)
{
	int size = 0;
	...
	switch(errno)
	{
		case DSM_LPSENSOR_AP3624_I2C_BUS_ERROR_NO:
		dsm_client_record(ap3426_sensor_dclient,"i2c_master_send or i2c_master_recv fail \n");
		break;
		default:
		break;
	}
...
EXIT_ERR:
	...
	mutex_unlock(&ap3xx6_obj->lock);
#ifdef CONFIG_HUAWEI_DSM
     ap3426_dsm_report_err(DSM_LPSENSOR_AP3624_I2C_BUS_ERROR_NO);
#endif
		return ret;
}
...
static int ap3xx6_init_client(struct i2c_client *client)
{
    ...
    ap3xx6_set_PSTtime(client, 0x06); //ps integrate time//modify by yifaguo 2016.07.29//0x0a
...
// *value = (obj->als)*628/1000;  //modify by yifaguo 2016.07.29
        *value = (obj->als)*578/1000;  //modify by yifaguo 2016.08.02
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
